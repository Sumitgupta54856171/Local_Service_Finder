...
export default function App() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        await axios.post('http://localhost:8000/api/service/create', {
    ...
    <div className="min-h-screen bg-slate-50 p-6 flex justify-center items-start pt-12 font-sans">
      <div className="w-full max-w-2xl bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col">
        ...
        <div className="px-8 pt-8 pb-4">
          <p className='text-sm text-slate-500 mb-2'>go to <Link href="/admin/dashboard" className='text-blue-600 hover:text-blue-700'>dashboard</Link></p>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight mb-2">Add New Service</h1>
          <p className="text-slate-500 text-sm">Fill in the primary details to register a new service location.</p>
        </div>
        {/* Success Message */}
        {isSubmitted && (
        ...
        <form onSubmit={formik.handleSubmit} className="flex flex-col gap-6 px-8 pb-8">
            ...
            <label className="block">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <option value="Shops" label="Shops" />
          ...
          <div className="space-y-4 pt-4">
            ...
            <label className="block">
              <span className="text-slate-700 text-sm font-medium mb-1.5 block">Initial Rating</span>
